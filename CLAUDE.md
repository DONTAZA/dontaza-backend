...
Service 계층 기능 구현이 완료되면 다음 두 작업을 서브에이전트로 **동시에** 실행한다:
1. **테스트 작성**: 서브에이전트가 해당 Service의 통합 테스트를 작성한다 (BDD 스타일, `@SpringBootTest`)
2. **API 스펙 문서 업데이트**: 서브에이전트가 `dontaza-wiki/api-spec.md`에 변경된 API를 반영한다

두 서브에이전트는 하나의 메시지에서 병렬로 실행하고, 완료 후 메인 에이전트가 빌드 검증 및 커밋한다.

...
**목표:** Service 구현 → 테스트 + API 스펙 병렬 작성 → 빌드 검증 → 자동 커밋 워크플로우를 자동화

**트리거:** 기능 구현, 수정, 테스트 작성, API 문서 업데이트 요청 시 `dontaza-dev` 스킬을 사용하라. 단순 질문은 직접 응답 가능.

**변경 이력:**
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-04-14 | 초기 구성 | 전체 | 기존 CLAUDE.md 병렬 작업 규칙을 하네스로 공식화 |
